...
import random
import itertools
from hbbrain.base.base_hmr_regressor import BaseHMRegressor, is_contain_missing_value, convert_format_missing_input_zero_one
from hbbrain.utils.membership_calc import membership_func_gfmm

class HMR_learner(BaseHMRegressor):
    def __init__ (self, theta = 0.1, _lambda = 1, V = None, W= None, d1 = None , d2 = None, r = None, N_samples=None, lambda_reg = 1e-4):
        super().__init__( theta, _lambda, V, W, d1, d2, r)
    ...
    def _fit (self, Xl, Xu, yl, yu):
        ...
        if (is_contain_missing_value(Xl) == True) or (is_contain_missing_value(Xu) == True):
            ...
            Xl, Xu, yl, yu = convert_format_missing_input_zero_one(
                Xl, Xu, yl, yu,
                np.nanmean(yl),
                np.nanmean(yu)
            )
        ...
        for i in range (n_samples):
            ...
            if is_exist_missing_value:
                membership_val = membership_func_gfmm(Xl[i,:], Xu[i,:],np.minimum(self.V, self.W), np.maximum (self.W, self.V), self._lambda)
            else: 
                membership_val = membership_func_gfmm(Xl[i,:], Xu[i,:],self.V,self.W, self._lambda)
            
            ranked_idx = np.argsort(membership_val)[::-1]
            is_expanded = False
            for idx in ranked_idx:
                V_new= np.minimum(self.V[idx], Xl[i,:])
                W_new = np.maximum (self.W[idx], Xu[i,:])

                # Expansion check (60%) dim:
                if np.mean((W_new - V_new) <= self.theta) >= 0.6:
                    self.V[idx] = V_new
                    self.W[idx] = W_new
            ...
            # Create new hyperbox if not expanded
            if not is_expanded:
        ...
        for i in range(n_samples):
            if is_exist_missing_value:
                membership_val_new = membership_func_gfmm(Xl[i,:], Xu[i,:],np.minimum(self.V, self.W), np.maximum (self.W, self.V), self._lambda)
            else:
                membership_val_new = membership_func_gfmm(Xl[i,:], Xu[i,:], self.V, self.W, self._lambda)
            # Normalize
