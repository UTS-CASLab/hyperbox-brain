...
Each base learner is trained by k-fold cross validation and random search based hyper-parameter tuning.
...
class ModelCombinationCrossValBagging(ClassifierMixin, BaseCrossValBagging):
    """A Bagging classifier of base hyperbox-based models trained on a full set of features and 
    a subset of samples. Each base learner is trained by k-fold cross validation and random search 
    based hyper-parameter tuning. Then, the hyperboxes from all base learners are combined to a single model.
    
    A model-level combination cross validation Bagging classifier of hyperbox-based models is an 
    ensemble meta-estimator that fits base hyperbox-based classifiers each 
    on random subsets of the original samples using k-fold cross validation and random search of 
    hyper-paramters, and then aggregate their hyperboxes into a single model and use this model
    for prediction. Such a meta-estimator can typically be used as
    ...
    def predict_proba(self, X):
        ...
        membership values of all representative hyperboxes of all classes in
        the prediction procedure using the final combined model.
    ...
    def get_n_hyperboxes_comb_model(self):
        ...
        -------
        int
            Total number of hyperboxes in the final combined model from all
            resulting hyperboxes of all base learners.
...
if __name__ == '__main__':
    ...
    model_comb_cross_val_bagging.fit(Xtr, ytr)
    print("Training time: %.3f (s)"%(model_comb_cross_val_bagging.elapsed_training_time))
    print('Number of hyperboxes in all base learners = %d'%model_comb_cross_val_bagging.get_n_hyperboxes())
    print('Number of hyperboxes in the combined model = %d'%model_comb_cross_val_bagging.get_n_hyperboxes_comb_model())
    
    y_pred_voting = model_comb_cross_val_bagging.predict_voting(Xtest)
    y_pred = model_comb_cross_val_bagging.predict(Xtest)
    ...
    # model_comb_cross_val_bagging.simple_pruning(X_val, y_val, 0.5, False)
    # print('Number of hyperboxes after pruning final model = %d'%model_comb_cross_val_bagging.get_n_hyperboxes_comb_model())
    
    # y_pred_2 = model_comb_cross_val_bagging.predict(Xtest)
