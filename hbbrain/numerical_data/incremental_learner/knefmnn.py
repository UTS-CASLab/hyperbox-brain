"""
Enhanced fuzzy min-max neural network classifier with k-nearest hyperboxes
expansion rules trained by the incremental learning algorithm.
"""
...
from hbbrain.utils.adjust_hyperbox import hyperbox_overlap_test_efmnn, hyperbox_contraction_efmnn
from hbbrain.utils.drawing_func import get_cmap, draw_box
from hbbrain.constants import MARKER_LIST


class KNEFMNNClassifier(BaseFMNNClassifier):
    """
    Enhanced fuzzy min-max neural network classifier with k-nearest hyperboxes
    expansion rules using the incremental learning algorithm.

    This class implements the enhanced online learning algorithm with k-nearest
    hyperboxes expansion rules to train the Simpson's fuzzy min-max neural
    network. Rather than creating a new hyperbox when the selected winner
    hyperbox is not satisfied with the expansion condition as in the enhanced
    fuzzy min-max neural network, this algorithm considers up to k winner
    hyperboxes. The creation of a new hyperbox only happens when all k selected
    winner hyperboxes cannot be extended to cover the input pattern. The details
    of this algorithm can be found in [1]_.
    ...
    k_neighbors : int, optional, default = 5
        Number of nearest hyperboxes used for in the consideraton of expansion
        rules.
    ...
    def fit(self, X, y):
        """
        Fit the model according to the given training data using the enhanced
        online learning algorithm with the k-nearest hyperbox selection rule.
        ...
        if len(self.C) > 0:
            # there are pre-trained hyperboxes, we need to add the class labels
            # to the current list of labels if they are not existed in this list
        ...
        if self.is_draw:
            ...
            drawing_canvas = self.initialise_canvas_graph(
                n_features, "Simpson FMNN - enhanced online learning with k-nearest hyperbox selection")
        ...
        for i in range(n_samples):
            ...
            if self.V.size == 0:
                ...
                if len(id_same_input_label_group) > 0:
                    ...
                    b = membership_func_fmnn(
                        X[i], V_sameX, W_sameX, self.gamma)

                    id_sorted_mem = np.argsort(b)[::-1]

                    if self.k_neighbors > len(id_sorted_mem):
                        n_hyperbox_candidates = len(id_sorted_mem)
                    else:
                        n_hyperbox_candidates = self.k_neighbors

                    is_winner_selected = False
                    for kk in range(n_hyperbox_candidates):
                        # store the index of the winner hyperbox in the list of all hyperboxes of all classes
                        j = id_same_input_label_group[id_sorted_mem[kk]]

                        if b[id_sorted_mem[kk]] != 1:
                            # Check for violation of max hyperbox size and class labels
                            Vj_new = np.minimum(self.V[j], X[i])
                            Wj_new = np.maximum(self.W[j], X[i])

                            if ((Wj_new - Vj_new) <= self.theta).all() == True:
                                ...
                                if self.V.shape[0] > 1:
                                    ...
                                    for ii in range(n_existed_hyperboxes):
                                        if (ii != id_of_winner_hyperbox) and self.C[ii] != self.C[id_of_winner_hyperbox]:
                                            # overlap test
                                            case_dim = hyperbox_overlap_test_efmnn(
                                                self.V, self.W, id_of_winner_hyperbox, ii, X[i])

                                            if case_dim.size > 0:
                                                self.V, self.W = hyperbox_contraction_efmnn(
                                                    self.V, self.W, case_dim, id_of_winner_hyperbox, ii)
                                                ...
                                                if self.is_draw:
                                                    ...
                                                    hyperboxes = draw_box(drawing_canvas, self.V[[ii, id_of_winner_hyperbox], 0:np.minimum(n_features, 3)], self.W[[
                                                                          ii, id_of_winner_hyperbox], 0:np.minimum(n_features, 3)], [boxii_color, boxwin_color])
                            ...
                            is_winner_selected = True  # kk is the winner hyperbox
                            break

                    if not is_winner_selected:
...
if __name__ == '__main__':
    ...
    optional.add_argument('--k_neighbors', type=int, default=5,
                          help='The number of nearest hyperboxes is considered for the hyperbox expansion process')
    ...
    if args.k_neighbors <= 0:
        parser.error("--k_neighbor has to be larger than 0")
    ...
    knefmnn_clf = KNEFMNNClassifier(
        theta=theta, gamma=gamma, k_neighbors=k_neighbors, is_draw=is_draw)
    knefmnn_clf.fit(Xtr, ytr)
    print('Number of hyperboxes = %d' % knefmnn_clf.get_n_hyperboxes())
    ...
    # y_pred_input_0, mem_val_classes, min_points_classes, max_points_classes = knefmnn_clf.get_sample_explanation(
    #     Xtest[sample_need_explain])
    # knefmnn_clf.show_sample_explanation(
    ...
    # knefmnn_clf.simple_pruning(X_val, y_val, 0.5, False)
    # print('Number of hyperboxes after pruning = %d'%knefmnn_clf.get_n_hyperboxes())
    # knefmnn_clf.draw_hyperbox_and_boundary()

    # y_pred_2 = knefmnn_clf.predict(Xtest)
