"""
Fuzzy min-max neural network classifier trained by the enhanced incremental
learning algorithm (EFMNN).
"""
...
class EFMNNClassifier(BaseFMNNClassifier):
    """
    Enhanced fuzzy min-max neural network classifier.

    This class implements an enhanced learning algorithm for Simpson's fuzzy
    min-max neural network. This algorithm use nine test cases for hyperbox
    overlap test and hyperbox contraction instead of four test cases in the
    original Simpson's fuzzy min-max neural network (FMNN). Additionally,
    this algorithm use the same hyperbox expansion condition regarding the
    maximum hyperbox size as the general fuzzy min-max neural network.
    The details of this algorithm can be found in [1]_.
    ...
    def fit(self, X, y):
        """
        Fit the fuzzy min-max neural network according to the given training
        data using the enhanced learning algorithm.
        ...
        if self.is_draw:
            ...
            drawing_canvas = self.initialise_canvas_graph(
                n_features, "Simpson FMNN - Enhanced Online learning")
                        ...
                        if not adjust:
                            ...
                            for ii in range(n_existed_hyperboxes):
                                if (ii != id_of_winner_hyperbox) and self.C[ii] != self.C[id_of_winner_hyperbox]:
                                    # overlap test
                                    case_dim = hyperbox_overlap_test_efmnn(self.V, self.W, id_of_winner_hyperbox, ii, X[i])

                                    if case_dim.size > 0:
                                        self.V, self.W = hyperbox_contraction_efmnn(self.V, self.W, case_dim, id_of_winner_hyperbox, ii)
...
if __name__ == '__main__':
    ...
    efmnn_clf = EFMNNClassifier(theta=theta, gamma=gamma, is_draw=is_draw)
    efmnn_clf.fit(Xtr, ytr)
    print('Number of hyperboxes = %d'%efmnn_clf.get_n_hyperboxes())
    
    y_pred = efmnn_clf.predict(Xtest)

    acc = accuracy_score(ytest, y_pred)
    print(f'Testing accuracy = {acc * 100: .2f}%')
    
    # sample_need_explain = 10
    # y_pred_input_0, mem_val_classes, min_points_classes, max_points_classes = efmnn_clf.get_sample_explanation(Xtest[sample_need_explain])
    # efmnn_clf.show_sample_explanation(Xtest[sample_need_explain], Xtest[sample_need_explain], min_points_classes, max_points_classes, y_pred_input_0, "2D")
    ...
    # efmnn_clf.simple_pruning(X_val, y_val, 0.5, False)
    # print('Number of hyperboxes after pruning = %d'%efmnn_clf.get_n_hyperboxes())
    # efmnn_clf.draw_hyperbox_and_boundary()
    
    # y_pred_2 = efmnn_clf.predict(Xtest)
